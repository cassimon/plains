...
def test_gas_quenching_parameters():
    ...
    assert "quenching_parameters" in perovskite_deposition, "No quenching_parameters section"
    ...
    assert gas_params["gas_type"] == "N2", f"Expected gas_type=N2, got {gas_params.get('gas_type')}"
    assert gas_params["pressure"] == 100000, f"Expected pressure=100000, got {gas_params.get('pressure')}"
    assert gas_params["flow_rate"] == 10, f"Expected flow_rate=10, got {gas_params.get('flow_rate')}"
    assert gas_params["height"] == 2.5, f"Expected height=2.5, got {gas_params.get('height')}"
    assert gas_params["nozzle_width"] == 5, f"Expected nozzle_width=5, got {gas_params.get('nozzle_width')}"
    assert gas_params["nozzle_form"] == "round", f"Expected nozzle_form=round, got {gas_params.get('nozzle_form')}"
        ...
        "generatedStacks": [
            {
                "combination": 1,
                "layers": [
                    {"id": "substrate-layer", "name": "substrate: Glass/FTO", "isSubstrate": True},
                    {
                        "id": "step-perovskite",
                        "name": "MAPbI3",
                        "layerType": "absorber",
